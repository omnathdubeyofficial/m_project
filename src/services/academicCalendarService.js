...
const createAcademicCalendar = async ({ program, from_date, to_date, start_time, end_time, program_details, pdf_file, host }) => {
  try {
    const createdData = await prisma.academic_calendar.create({
      data: { z_id: uuidv4(), program, from_date: setDateFormat(from_date), to_date: setDateFormat(to_date), start_time, end_time, program_details, pdf_file, host, cdate: setUserDate(), ctime: setUserTime() },
    });
...
const updateAcademicCalendar = async ({ z_id, program, from_date, to_date, start_time, end_time, program_details, pdf_file, host }) => {
  try {
    ...
    const updatedData = await prisma.academic_calendar.update({
      where: { z_id },
      data: { program, from_date: setDateFormat(from_date), to_date: setDateFormat(to_date), start_time, end_time, program_details, pdf_file, host, udate: setUserDate(), utime: setUserTime() },
    });
