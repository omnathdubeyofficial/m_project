...
import { GET_HOLIDAY_LISTS_DATA } from "../../query/HolidayListQuery/fetchHolidayLists";
...
const Holiday_List = () => {
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      try {
        const response = await executeQuery(GET_HOLIDAY_LISTS_DATA);
        setAdmins(response?.getHolidayLists || []);
      } catch (error) {
  ...
  const handleDelete = useCallback(async (id) => {
    try {
      const response = await executeMutation(DELETE_HOLIDAY_LIST_MUTATION, { z_id: id });
      if (response?.deleteHolidayLists?.success_msg) {
        setAdmins(prev => prev.filter(admin => admin.z_id !== id));

        // ✅ Success popup
        setPopupMessage({ text: response.deleteHolidayLists.success_msg, type: "success" });
        setShowPopup(true);
        setTimeout(() => setShowPopup(false), 3000);
      } else {
        // ❌ Error popup
        setPopupMessage({ text: "Failed to delete: " + response?.deleteHolidayLists?.error_msg, type: "error" });
        setShowPopup(true);
        setTimeout(() => setShowPopup(false), 3000);
      }
    } catch (error) {
    ...
    <div className="bg-gray-100 min-h-screen pt-28">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
        ...
        <div className="relative flex items-center border w-full sm:w-80">
          ...
          <Link href="/University_Management/Holiday_Form" className="bg-blue-600 text-white flex items-center px-4 py-2  hover:bg-blue-700 transition">
         ...
         <table className="min-w-full bg-white border border-gray-300">
           <thead className="bg-gradient-to-r from-blue-900 via-blue-600 to-blue-900 border border-gray-300">
             <tr className="text-white text-sm font-semibold border border-gray-300">
               ...
               <th className="px-6 py-3 text-left border border-gray-300">day</th>
               <th className="px-6 py-3 text-left border border-gray-300">holiday_name</th>
               <th className="px-6 py-3 text-left border border-gray-300">details</th>
             ...
             {currentAdmins.map((admin) => (
               <tr key={admin.z_id} className="hover:bg-gray-50 transition duration-200 border border-gray-300">
                 ...
                 <td className="px-6 py-4 truncate max-w-[200px] border border-gray-300">
                   <span className="text-gray-600">{admin.from_date}</span>
                 </td>
