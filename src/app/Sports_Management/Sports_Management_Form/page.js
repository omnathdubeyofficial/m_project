...
const Sports_Management_Form = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      setFormData({ ...formData, sportBanner: URL.createObjectURL(file) });
    }
  };

  const handleClearFile = () => {
    setFormData({ ...formData, sportBanner: null });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Sports Data Submitted:", formData);
  };
      ...
      <div className="container mx-auto py-10 px-4 sm:px-6 md:px-8 max-w-8xl pt-32">
        <div className="bg-white shadow-lg p-6 sm:p-8 w-full relative">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-5">
            ...
            <h2 className="text-2xl text-center text-gray-800">Sports Management Form</h2>
          </div>

          <form onSubmit={handleSubmit} className="grid gap-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                { name: "sportName", label: "Sport Name" },
                { name: "coachName", label: "Coach Name" },
                { name: "venue", label: "Venue" },
                { name: "teamName", label: "Team Name" },
                { name: "numberOfPlayers", label: "Number of Players" },
              ].map((field, index) => (
            ...
            <div className="flex flex-col items-center border p-4 rounded-lg w-full relative mt-4">
              <label className="font-semibold text-lg mb-2">Upload Sport Banner</label>
...
export default Sports_Management_Form;
