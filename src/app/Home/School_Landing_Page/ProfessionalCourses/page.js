...
import { Star, Tag, Users, UserCheck, UserMinus } from "lucide-react";
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 auto-rows-min">
          {classes.map((cls, index) => {
            ...
            return (
              <div
                key={index}
                className="relative bg-white p-6 shadow-lg text-left transition-all duration-500 ease-in-out transform shadow-md scale-100 min-h-[300px] border-b border-gray-300"
              >
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {cls.tags && typeof cls.tags === "string" ? (
                    cls.tags.split(",").map((tag, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 text-sm bg-blue-100 rounded-full flex items-center gap-1"
                      >
