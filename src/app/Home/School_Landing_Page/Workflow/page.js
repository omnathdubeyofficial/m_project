...
export default function VaekonSchoolWorkflow() {
  return (
    <div className="bg-[#FDF8E1] py-20 sm:py-32">
    <div className="max-w-7xl mx-auto px-6 lg:px-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="text-left">
          <h2 className="text-lg font-semibold text-[#D97706]">Empowering Future Leaders</h2>
          <p className="mt-4 text-4xl font-semibold text-[#3B3B3B] sm:text-5xl">Vaekon School - Excellence in Education</p>
          ...
          <div className="mt-10 space-y-6">
            {features.map((feature) => (
              <div key={feature.name} className="flex items-start space-x-4">
                <feature.icon className="size-10 text-[#D97706]" aria-hidden="true" />
                <div>
                  <h3 className="text-lg font-semibold text-[#3B3B3B]">{feature.name}</h3>
        ...
        <div className="relative">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-[#D97706] to-transparent opacity-20 rounded-3xl" />
        </div>
      </div>
    </div>
  </div>
